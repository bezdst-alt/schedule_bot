...
async def cmd_list_employees(message):
    ...
    emojis = {"2x2_day":"☀️","24x3":"🌙","2-1-2-3":"🔄"}
    lines = ["👥 **Список сотрудников:**\n"]
    for e in employees:
        em = emojis.get(e[2], "👤")
        lines.append(f"{em} {e[1]} — {GRAPHIC_NAMES.get(e[2],e[2])} (с {e[3]})")
...
def format_schedule(for_date, employees):
    ...
    if day_workers:
        for w in sorted(day_workers):
            ...
            s = " ⭐️" if emp and (emp[0],date_str,"day") in overrides else ""
            lines.append(f"   👤 {w}{s}")
    else:
        lines.append("   ❌ Нет")
    lines.append("")
    ...
    if night_workers:
        for w in sorted(night_workers):
            emp = next((e for e in employees if e[1]==w), None)
            s = " ⭐️" if emp and (emp[0],date_str,"night") in overrides else ""
            lines.append(f"   👤 {w}{s}")
    else:
        lines.append("   ❌ Нет")
    return "\n".join(lines)
...
def format_week_schedule(start_date, employees):
    ...
    lines.append(f"🗓 С {fmt_date(start_date)}")
    lines.append("")
    for i in range(7):
        ...
        lines.append(f"━━━ **{fmt_date_full(d)}** ━━━")
        lines.append("")
        lines.append("☀️ **День:**")
        if day_workers:
            for w in sorted(day_workers):
...
async def process_schedule_query(callback):
    ...
    if callback.data == "sched_today":
        ...
        await callback.message.edit_text(format_week_schedule(today, employees), parse_mode="Markdown")
...
async def cmd_add_employee(message):
    if not await admin_only(message):
        return
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="☀️ 2x2 день", callback_data="graphic_2x2_day")],
    ...
    await message.answer("👤 **Выберите график:**", reply_markup=kb, parse_mode="Markdown")
...
# Остальные команды (remove, override, recipients, рассылки)
# они такие же как в предыдущей версии, но я их сократил для краткости
...
async def cmd_remove_emp(message):
    ...
    employees = db.list_employees()
    if not employees:
        await message.answer("📭 Нет сотрудников."); return
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=f"❌ {e[1]}", callback_data=f"rememp_{e[0]}")] for e in employees
    ])
    await message.answer("🗑 **Выберите сотрудника:**", reply_markup=kb, parse_mode="Markdown")
...
async def proc_rem_emp(callback):
    ...
    if emp:
        ...
        await callback.message.edit_text(f"🗑 **{emp[1]}** удалён.")
    await callback.answer()
...
async def cmd_add_ov(message):
    ...
    employees = db.list_employees()
    if not employees:
        await message.answer("📭 Сначала добавьте сотрудников."); return
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=e[1], callback_data=f"overempe_{e[0]}")] for e in employees
    ])
    await message.answer("⭐ **Выберите сотрудника для РВ:**", reply_markup=kb, parse_mode="Markdown")

@dp.callback_query(lambda c: c.data.startswith("overempe_"))
async def proc_ov_emp(callback):
    eid = int(callback.data.replace("overempe_",""))
    user_data[callback.from_user.id] = {'action':'add_override','employee_id':eid,'step':'date'}
...
async def proc_ov_date(message):
    ...
    await message.answer("⭐ **Тип смены:**", reply_markup=kb, parse_mode="Markdown")
...
async def proc_ov_shift(callback):
    ...
    ok = db.add_override(data.get('employee_id'), data.get('date'), shift)
    if ok:
        t = "☀️ дневную" if shift=="day" else "🌙 ночную"
        await callback.message.edit_text(f"✅ РВ на **{t}** добавлено!")
    else:
        await callback.message.edit_text("❌ Ошибка.")
    if callback.from_user.id in user_data: del user_data[callback.from_user.id]
    await callback.answer()

@dp.message(Command("add_recipient"))
async def cmd_add_recip(message):
    if not await admin_only(message): return
    user_data[message.from_user.id] = {'action':'add_recip','step':'id'}
...
async def proc_recip_id(message):
    uid = message.from_user.id
    uid_to_add = None; name = ""
    if message.forward_from:
        uid_to_add = message.forward_from.id; name = message.forward_from.full_name or f"User {uid_to_add}"
    elif message.text and message.text.strip().isdigit():
        uid_to_add = int(message.text.strip()); name = f"User {uid_to_add}"
    else:
        await message.answer("❌ Не удалось определить ID."); return
    ok = db.add_recipient(uid_to_add, name)
    if ok: await message.answer(f"✅ Получатель добавлен! ID: {uid_to_add}")
    else: await message.answer("❌ Ошибка.")
    del user_data[uid]

@dp.message(Command("list_recipients"))
async def cmd_list_recip(message):
    if not await admin_only(message): return
    recipients = db.list_recipients()
    if not recipients: await message.answer("📭 Нет получателей."); return
    lines = ["📢 **Получатели:**\n"] + [f"👤 {r[1]} (ID: {r[0]})" for r in recipients]
    await message.answer("\n".join(lines), parse_mode="Markdown")
...
async def send_morning_schedule():
    ...
    for o in ov:
        emp = next((e for e in employees if e[1]==o[1]), None)
        if emp: overrides.add((emp[0], ds, o[3]))
    ...
    if day_w:
        for w in sorted(day_w): text += f"   👤 {w}\n"
    else: text += "   ❌ Нет\n"
    text += "\n🌙 **Ночь (20-08):**\n"
    if night_w:
        for w in sorted(night_w): text += f"   👤 {w}\n"
    else: text += "   ❌ Нет\n"
    for r in db.list_recipients():
        try: await bot.send_message(r[0], text, parse_mode="Markdown")
        except: pass
...
async def send_evening_schedule():
    ...
    for o in ov2:
        emp = next((e for e in employees if e[1]==o[1]), None)
        if emp: overrides2.add((emp[0], tms, o[3]))
    day_w = list(set(get_day_schedule(tomorrow, employees, overrides2)))
    text = f"🌆 **Добрый вечер!**\n\n🌙 **Сегодня ночью:**\n"
    if night_w:
        for w in sorted(night_w): text += f"   👤 {w}\n"
    else: text += "   ❌ Нет\n"
    text += f"\n☀️ **Завтра днём ({fmt_date_full(tomorrow)}):**\n"
    if day_w:
        for w in sorted(day_w): text += f"   👤 {w}\n"
    else: text += "   ❌ Нет\n"
    for r in db.list_recipients():
