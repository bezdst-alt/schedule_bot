...
async def cmd_start(message):
    if not await admin_only(message):
        return
    await message.answer("Привет! Команды: /schedule /add_employee /add_employees_bulk /remove_employee /list_employees /add_override /remove_override /add_recipient /remove_recipient /list_recipients /date_schedule")
...
async def cmd_schedule(message):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="Cегодня", callback_data="sched_today")],
...
def format_week_schedule(start_date, employees):
    lines = []
    lines.append("НЕДЕЛЯ С " + fmt_date(start_date))
    lines.append("")
    for i in range(7):
...
async def process_schedule_query(callback):
    ...
    if callback.data == "sched_today":
        ...
        await callback.message.edit_text("Введите дату (ДД.ММ.ГГГГ) или диапазон:")
    await callback.answer()
...
async def cmd_date_schedule(message):
    ...
    await message.answer("Введите дату (ДД.ММ.ГГГГ) или диапазон:")
...
async def cmd_add_employees_bulk(message):
    if not await admin_only(message):
        return
    await message.answer("Отправьте список в формате:\nИмя - график (с ГГГГ-ММ-ДД)")
...
async def cmd_remove_employee(message):
    if not await admin_only(message):
        return
    employees = db.list_employees()
    if not employees:
...
async def process_remove_employee(callback):
...
async def cmd_add_override(message):
    if not await admin_only(message):
        return
    employees = db.list_employees()
    if not employees:
        ...
        return
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=e[1], callback_data=f"overemp_{e[0]}")] for e in employees
    ])
    await message.answer("Выберите сотрудника:", reply_markup=kb)

@dp.callback_query(lambda c: c.data.startswith("overemp_"))
async def process_override_emp(callback):
    eid = int(callback.data.replace("overemp_",""))
...
async def process_override_date(message):
    uid = message.from_user.id
    try:
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="Дневная", callback_data="overshift_day")],
        [InlineKeyboardButton(text="Ночная", callback_data="overshift_night")],
    ])
    await message.answer("Тип смены:", reply_markup=kb)

@dp.callback_query(lambda c: c.data.startswith("overshift_"))
async def process_override_shift(callback):
    shift = callback.data.replace("overshift_","")
...
async def cmd_remove_override(message):
    if not await admin_only(message):
        return
    employees = db.list_employees()
    if not employees:
        ...
        return
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=e[1], callback_data=f"removr_{e[0]}")] for e in employees
    ])
    await message.answer("Выберите:", reply_markup=kb)

@dp.callback_query(lambda c: c.data.startswith("removr_"))
async def proc_rem_ovr_emp(callback):
    eid = int(callback.data.replace("removr_",""))
    user_data[callback.from_user.id] = {'action':'remove_override','employee_id':eid,'step':'date'}
    await callback.message.edit_text("Введите дату удаления РВ (ДД.ММ.ГГГГ):")
    await callback.answer()
...
async def proc_rem_ovr_date(message):
    uid = message.from_user.id
    try:
        parsed = datetime.strptime(message.text.strip(), "%d.%m.%Y")
    except:
