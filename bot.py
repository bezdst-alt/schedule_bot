...
async def emp_add(callback):
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="2x2 день", callback_data="g_2x2_day")],
...
async def rememp_process(callback):
    ...
    if emp:
        ...
        await callback.message.edit_text(f"Удален: {emp[1]}", reply_markup=employees_menu())
    await callback.answer()
...
async def ov_add_shift(callback):
    ...
    if ok:
        await callback.message.edit_text("PB добавлено!", reply_markup=overrides_menu())
    else:
...
async def cmd_date_schedule(message):
    if not await admin_only(message):
        return
    user_data[message.from_user.id] = {'action':'sched_custom'}
...
async def rec_remove_process(callback):
    ...
    await callback.message.edit_text("Удален.", reply_markup=recipients_menu())
    await callback.answer()
